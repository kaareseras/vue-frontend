...
const deleteJob = async () => {
  try {
    ...
    if (confirmDelete) { // Check if the user confirmed the deletion
      await axios.delete(`${import.meta.env.VITE_API_URL}/jobs/${jobId}`);
      toast.success('Job deleted successfully!');
      router.push('/jobs'); // Redirect to the jobs list after deletion
    }
  } catch (error) {
        ...
        <aside>
          ...
          <div class="bg-white p-6 rounded-lg shadow-md mt-6">
            ...
            <RouterLink :to="`/jobs/edit/${state.job.id}`"
              class="bg-green-500 hover:bg-green-600 text-white text-center font-bold py-2 px-4 rounded-full w-full focus:outline-none focus:shadow-outline mt-4 block">Edit Job
            </RouterLink>
